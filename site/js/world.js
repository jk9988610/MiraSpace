...
/**
 * Simulation world: fixed-tick loop, fields, particles, metrics.
 */
export class World {
  /**
   * @param {object} preset
  ...
  constructor(preset, seedOverride) {
    ...
    this.metrics = new Metrics(preset, this.particles.typeCountsSnapshot());
  ...
  tick() {
    ...
    const events = this.particles.step(this.dt, this.fields, this.rng);
    this.metrics.record(this.tickCount, this.simTime, this.particles, events);
  }
